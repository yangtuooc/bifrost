...
export function PrometheusFormFragment({
	...
	const renderActions = (tabKey: "pull" | "push", tabDirty: boolean, onResetTab: () => void) => {
		...
		const otherTabLabel = tabKey === "pull" ? t("observability.prometheus.tabs.pushBased") : t("observability.prometheus.tabs.pullBased");
		const saveDisabled = !hasPrometheusAccess || !tabDirty || formIsInvalid;
		let tooltipMsg = "";
		if (!tabDirty) {
			tooltipMsg = t("observability.prometheus.noChangesInTab");
		} else if (formIsInvalid && otherTabHasErrors && !thisTabHasErrors) {
			tooltipMsg = t("observability.prometheus.fixOtherTab", { tab: otherTabLabel });
		} else if (formIsInvalid) {
			tooltipMsg = t("observability.common.fixValidationErrors");
		}
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<Button type="submit" disabled={saveDisabled} isLoading={isSaving} data-testid={`prometheus-${tabKey}-save-btn`}>
									{t("observability.prometheus.save")}
								</Button>
							</TooltipTrigger>
							{tooltipMsg && (
								<TooltipContent>
				...
				<Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "pull" | "push")}>
					<TabsList className="gap-2">
						<TabsTrigger value="pull" className="px-2 py-1" data-testid="prometheus-tab-pull">
							{t("observability.prometheus.tabs.pullBased")}
						</TabsTrigger>
						<TabsTrigger value="push" className="px-2 py-1" data-testid="prometheus-tab-push">
							{t("observability.prometheus.tabs.pushBased")}
						</TabsTrigger>
					</TabsList>
					...
					<TabsContent value="pull" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="text-sm font-medium">{t("observability.prometheus.pullTitle")}</h3>
								<p className="text-muted-foreground text-xs">{t("observability.prometheus.pullDescription")}</p>
							</div>
							<FormField
						...
						<div className="bg-muted/50 rounded-md p-4">
							<div className="flex items-center justify-between">
								<div className="flex flex-col gap-1">
									<span className="text-sm font-medium">{t("observability.prometheus.metricsEndpoint")}</span>
							...
							<p className="text-muted-foreground mt-2 text-xs">{t("observability.prometheus.scrapeDescription")}</p>
						</div>
					...
					<TabsContent value="push" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="flex flex-row items-center gap-2 text-sm font-medium">
									{t("observability.prometheus.pushTitle")} <Badge variant="secondary">{t("observability.prometheus.betaBadge")}</Badge>
								</h3>
								<p className="text-muted-foreground text-xs">{t("observability.prometheus.pushDescription")}</p>
							</div>
							<FormField
								control={form.control}
						...
						<Alert variant="info">
							...
							<AlertDescription className="text-xs">{t("observability.prometheus.pushGatewayWarning")}</AlertDescription>
						</Alert>
						...
						<div className="space-y-4">
							<FormField
								...
								render={({ field }) => (
									<FormItem className="w-full">
										<FormLabel>{t("observability.prometheus.pushGatewayUrl")}</FormLabel>
										<FormControl>
											<SecretVarInput
												placeholder={t("observability.prometheus.pushGatewayUrlPlaceholder")}
												disabled={!hasPrometheusAccess}
												data-testid="prometheus-push-gateway-url"
												{...field}
											/>
										</FormControl>
										<FormDescription>{t("observability.prometheus.pushGatewayUrlDescription")}</FormDescription>
										<FormMessage />
									</FormItem>
								)}
							/>
							...
							<div className="grid grid-cols-2 gap-4">
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("observability.prometheus.jobName")}</FormLabel>
											<FormControl>
												<Input placeholder="bifrost" disabled={!hasPrometheusAccess} data-testid="prometheus-job-name" {...field} />
											</FormControl>
											<FormDescription>{t("observability.prometheus.jobNameDescription")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("observability.prometheus.pushInterval")}</FormLabel>
											<FormControl>
												<Input
													type="number"
													min={1}
											...
											<FormDescription>{t("observability.prometheus.pushIntervalDescription")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className="flex items-center gap-2">
											{t("observability.prometheus.instanceId")}
											<TooltipProvider>
												<Tooltip>
													<TooltipTrigger asChild>
													...
													<TooltipContent>
														<p className="max-w-xs text-xs">{t("observability.prometheus.instanceIdTooltip")}</p>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</FormLabel>
										<FormControl>
											<Input
												placeholder={t("observability.prometheus.instanceIdPlaceholder")}
												disabled={!hasPrometheusAccess}
							...
							<div className="space-y-4 border-t pt-4">
								{!showBasicAuth ? (
									...
									>
										...
										{t("observability.prometheus.addBasicAuth")}
									</Button>
								) : (
									<>
										<div className="flex items-center justify-between">
											<span className="text-sm font-medium">{t("observability.prometheus.basicAuthentication")}</span>
											<Button
												type="button"
												variant="ghost"
												size="sm"
												...
												aria-label={t("observability.prometheus.removeBasicAuth")}
											>
										...
										<div className="border-muted grid grid-cols-2 gap-4">
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("observability.prometheus.username")}</FormLabel>
														<FormControl>
															<SecretVarInput
																placeholder={t("observability.prometheus.usernamePlaceholder")}
																disabled={!hasPrometheusAccess}
											...
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("observability.prometheus.password")}</FormLabel>
														<FormControl>
															<SecretVarInput
																type="password"
																placeholder={t("observability.prometheus.passwordPlaceholder")}
																disabled={!hasPrometheusAccess}
																hideValueWhenEnv
																redactNonEnvValue
