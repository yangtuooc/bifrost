	...
	useEffect(() => {
		...
		getProvider(provider)
			...
			.catch((err) => {
				...
				toast.error(t("providers.somethingWentWrong"), {
					description: t("providers.getProviderConfigError", { message: getErrorMessage(err) }),
				});
			});
...
function ProviderStatusBadge({ status }: { status: ProviderStatus }) {
	...
	return status != "active" ? (
		<Tooltip>
			...
			<TooltipContent>{status === "error" ? t("providers.status.couldNotInitialize") : t("providers.status.deleted")}</TooltipContent>
		</Tooltip>
	) : null;
}
...
function KeyDiscoveryFailedBadge({
	...
	return (
		<Tooltip>
			...
			<TooltipContent>{provider.description || t("providers.status.modelDiscoveryFailed")}</TooltipContent>
		</Tooltip>
	);
}