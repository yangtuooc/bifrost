	...
	const loadSessionPage = useCallback(
		async (offset: number, reset = false) => {
			...
			try {
				...
				if (result.error) {
					toast.error(t("logs.sessionDetails.loadFailed"), {
		...
		[onOpenChange, sessionId, sortOrder, sortSessionLogs, t, triggerGetSession],
	);
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-full flex-col gap-4 overflow-x-hidden p-8 sm:max-w-[60%]">
				<div className="flex items-center justify-between gap-4">
					<div>
						<div className="text-lg font-medium">{t("logs.sessionDetails.title")}</div>
						{sessionId && onFilterByParentRequestId ? (
							<Tooltip>
								...
								<TooltipContent sideOffset={6}>{t("logs.sessionDetails.filterThisSession")}</TooltipContent>
							</Tooltip>
						) : (
					...
					<div className="flex items-center gap-3">
						...
						>
							...
							{sortOrder === "asc" ? t("logs.sessionDetails.earliestFirst") : t("logs.sessionDetails.latestFirst")}
						</Button>
					</div>
				</div>
					...
					<Table containerClassName="h-full overflow-auto">
						<TableHeader className="sticky top-0 z-10 bg-[#f9f9f9] dark:bg-[#27272a]">
							<TableRow>
								...
								<TableHead>{t("logs.table.type")}</TableHead>
				...
				{hasMore ? (
					<div className="flex justify-center">
						...
						>
							...
							{t("logs.sessionDetails.loadMore")}
						</Button>
					</div>
				) : null}
			</SheetContent>
