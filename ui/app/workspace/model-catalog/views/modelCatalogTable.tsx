...
export default function ModelCatalogTable({
	...
	const summaryCards = [
		{ label: t("modelCatalog.summary.totalProviders"), value: totalProviders.toLocaleString() },
		{ label: t("modelCatalog.summary.totalModels"), value: totalModels.toLocaleString() },
		{ label: t("modelCatalog.summary.totalRequests24h"), value: totalRequests24h.toLocaleString() },
		{ label: t("modelCatalog.summary.totalCost24h"), value: formatCost(totalCost24h) },
	];
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					<h2 className="text-lg font-semibold">{t("modelCatalog.table.title")}</h2>
					<p className="text-muted-foreground text-sm">{t("modelCatalog.table.description")}</p>
				</div>
				<Select
							...
							<TableHead>
								<TooltipProvider>
									<div className="flex items-center gap-1">
										{t("modelCatalog.table.models")}
										<Tooltip>
											<TooltipTrigger data-testid="model-catalog-models-info-trigger">
												<Info className="text-muted-foreground h-3.5 w-3.5" />
											</TooltipTrigger>
											<TooltipContent side="bottom">{t("modelCatalog.table.modelsTooltip")}</TooltipContent>
										</Tooltip>
									</div>
								</TooltipProvider>
							</TableHead>
							<TableHead className="text-right">{t("modelCatalog.table.totalTraffic24h")}</TableHead>
							<TableHead className="text-right">{t("modelCatalog.table.totalCost24h")}</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
						{rows.length === 0 ? (
							<TableRow>
								<TableCell colSpan={4} className="h-24 text-center">
									<span className="text-muted-foreground text-sm">{t("modelCatalog.table.noMatchingProviders")}</span>
								</TableCell>
							</TableRow>
						) : (
							rows.map((row) => (
					...
					<Tooltip>
						<TooltipTrigger data-testid="model-catalog-models-overflow-trigger">
							<Badge variant="outline" className="text-xs font-normal">
								{t("modelCatalog.table.moreModels", { count: remaining })}
							</Badge>
						</TooltipTrigger>
