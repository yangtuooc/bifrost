		...
		<DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
			<DropdownMenuTrigger asChild onClick={(event) => event.stopPropagation()}>
				<Button
					...
					size="icon"
					className="h-8 w-8"
					aria-label={t("customPricing.overrides.actions.actionsFor", { name: row.name || row.id })}
...
// 返回顶层作用域标签：全局或 Virtual Key。
function scopeLabel(override: PricingOverride, _virtualKeyMap: Map<string, string>, t: TFunction): string {
	const scopeKind = resolveScopeKind(override);
	if (override.virtual_key_id && scopeKind.startsWith("virtual_key")) {
		return t("customPricing.overrides.scope.virtualKey");
	}
	return t("customPricing.overrides.scope.global");
}

// 返回覆盖规则的 key 标签；没有限定具体 key 时返回 "-"。
function keyLabel(override: PricingOverride, keyLabelMap: Map<string, string>, t: TFunction): string {
	if (!override.provider_key_id) {
		if (!override.provider_id) return "-";
		return t("customPricing.overrides.scope.allKeys");
	}
...
export default function ScopedPricingOverridesView() {
	...
	useEffect(() => {
		if (error) {
			toast.error(t("customPricing.overrides.toasts.loadFailed"), { description: getErrorMessage(error) });
		}
	}, [error, t]);
	...
	const createScopeLock = useMemo(() => {
		...
		return {
			...
			providerID: providerID || undefined,
			providerKeyID: providerKeyID || undefined,
			label: `${t(`customPricing.overrides.scopeKinds.${scopeKind}`)}${virtualKeyID || providerID || providerKeyID ? ` ${t("customPricing.overrides.filters.filtered")}` : ""}`,
		};
	}, [scopeKind, virtualKeyID, providerID, providerKeyID, t]);
	...
	const handleDeleteConfirm = async () => {
		...
		try {
			...
			toast.success(t("customPricing.overrides.toasts.deleted"));
			setDeleteTarget(null);
		} catch (deleteError) {
			toast.error(t("customPricing.overrides.toasts.deleteFailed"), { description: getErrorMessage(deleteError) });
		}
	};
	...
	return (
		<div className="flex flex-col overflow-y-auto">
			<div className="mb-4 flex items-center justify-between gap-4">
				<div>
					<h2 className="text-lg font-semibold tracking-tight">{t("customPricing.overrides.title")}</h2>
					<p className="text-muted-foreground text-sm">{t("customPricing.overrides.description")}</p>
				</div>
				<Button data-testid="pricing-override-create-btn" onClick={openCreateDrawer} className="gap-2">
					<Plus className="h-4 w-4" />
					<span className="hidden sm:inline">{t("customPricing.overrides.actions.new")}</span>
				</Button>
			</div>
			...
			<div className="relative mb-4 max-w-sm">
				...
				<Input
					aria-label={t("customPricing.overrides.filters.searchAria")}
					placeholder={t("customPricing.overrides.filters.searchPlaceholder")}
					value={search}
			...
			<div className="mb-2 overflow-hidden rounded-sm border">
				{isLoading ? (
					<div className="p-4 text-sm">{t("customPricing.overrides.table.loading")}</div>
				) : error ? (
					<div className="p-4 text-sm text-red-500">{t("customPricing.overrides.table.loadFailed")}</div>
				) : (
					<Table containerClassName="h-full overflow-auto">
						<TableHeader className="bg-muted sticky top-0 z-10">
							<TableRow className="bg-muted/50">
								<TableHead className="font-semibold">{t("common.table.name")}</TableHead>
								<TableHead className="font-semibold">{t("customPricing.overrides.table.scope")}</TableHead>
								<TableHead className="font-semibold">{t("customPricing.overrides.table.provider")}</TableHead>
								<TableHead className="font-semibold">{t("customPricing.overrides.table.key")}</TableHead>
								<TableHead className="font-semibold">{t("customPricing.overrides.table.model")}</TableHead>
							...
							{rows.length === 0 ? (
								<TableRow>
									<TableCell colSpan={6} className="h-24 text-center">
										<span className="text-muted-foreground text-sm">{t("customPricing.overrides.table.noMatching")}</span>
									</TableCell>
								</TableRow>
							) : (
								rows.map((row) => (
									<TableRow key={row.id} className="group hover:bg-muted/50 cursor-pointer transition-colors">
										...
										<TableCell>
											<Badge variant="secondary">{scopeLabel(row, virtualKeyMap, t)}</Badge>
										</TableCell>
										<TableCell>
											{(() => {
										...
										>
											<div className="flex items-center justify-center">
												<PricingOverrideActionsMenu row={row} onEdit={openEditDrawer} onDelete={setDeleteTarget} t={t} />
											</div>
										</TableCell>
									</TableRow>
								))
			...
			<AlertDialog open={!!deleteTarget} onOpenChange={(open) => (!open ? setDeleteTarget(null) : undefined)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>{t("customPricing.overrides.deleteDialog.title")}</AlertDialogTitle>
						<AlertDialogDescription>
							{t("customPricing.overrides.deleteDialog.description", { name: deleteTarget?.name })}
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
