...
export function FilePreview({
	...
	if (source.unavailable && kind !== "text") {
		return (
			<FallbackBlock fileName={fileName} file={file} downloadUrl={resolvedDownloadUrl}>
				{t("skillsRepo.preview.availableAfterSaving")}
			</FallbackBlock>
		);
	}
	...
	if (kind === "text") {
		...
		if (fetchState === "error") {
			return (
				<FallbackBlock fileName={fileName} file={file} downloadUrl={resolvedDownloadUrl}>
					{t("skillsRepo.preview.couldNotLoad")}
				</FallbackBlock>
			);
		}
		...
		return (
			<ScrollArea className="h-full" bidirectionalScroll>
				<pre className="p-4 font-mono text-xs leading-5 whitespace-pre-wrap">{textContent || t("skillsRepo.shared.emptyValue")}</pre>
			</ScrollArea>
		);
	}
		...
		<div className="flex h-full min-h-0 flex-col overflow-hidden rounded-sm border">
			<div className="bg-muted/30 flex h-9 shrink-0 items-center justify-between gap-2 border-b px-3">
				<span className="flex min-w-0 items-center gap-1.5 truncate font-mono text-xs" title={file.path}>
					{file.path}
					{dirty && (
						<span className="bg-primary inline-block size-1.5 shrink-0 rounded-full" aria-label={t("skillsRepo.preview.unsavedChanges")} />
					)}
				</span>
				<div className="flex shrink-0 items-center gap-1">
					{isEditable && (
						...
						>
							...
							{t("skillsRepo.preview.saveFile")}
						</Button>
					)}
					{resolved && (
						<Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground h-7 px-2" asChild>
							<a href={resolved} download={fileName} aria-label={t("skillsRepo.preview.downloadAria", { file: fileName })}>
			...
			<div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
				...
				<div className="flex flex-col gap-0.5">
					...
					<p className="text-muted-foreground text-xs">
						{file.mime_type || t("skillsRepo.preview.uploadedFile")}
						{file.file_size_bytes ? ` · ${formatFileSize(file.file_size_bytes)}` : ""}
					</p>
				</div>
				<p className="text-muted-foreground max-w-md text-xs">{t("skillsRepo.preview.uploadReadOnly")}</p>
			</div>
		);
	}
		...
		return (
			<div className="flex h-full flex-col gap-4 p-4">
				<div className="flex flex-col gap-1.5">
					<Label className="text-muted-foreground text-xs">{t("skillsRepo.files.sourceUrl")}</Label>
					<Input
	...
	if (file.source_type === "dataurl") {
		...
		if (!file.dataurl && kind !== "text") {
			return (
				<FallbackBlock fileName={fileName} file={file} downloadUrl={source.url}>
					{t("skillsRepo.preview.binaryDataUrlReadOnly")}
				</FallbackBlock>
			);
		}
		...
		return (
			<div className="flex h-full min-h-0 flex-col gap-2 p-4">
				<Label className="text-muted-foreground text-xs">{t("skillsRepo.files.dataUrl")}</Label>
				<Textarea
	...
	return (
		<div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
			...
			<div className="flex flex-col gap-0.5">
				<p className="max-w-full truncate font-mono text-sm">{fileName}</p>
				<p className="text-muted-foreground text-xs">
					{file.mime_type || t("skillsRepo.preview.unknownType")}
					{file.file_size_bytes ? ` · ${formatFileSize(file.file_size_bytes)}` : ""}
				</p>
			</div>
