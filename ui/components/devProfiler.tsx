	...
	return (
		<div className="p-3">
			...
			<div className="mb-2 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					{goroutineHealth === "healthy" && (
						<span className="rounded bg-emerald-500/20 px-1.5 py-0.5 text-[10px] text-emerald-400">{t("common.status.healthy")}</span>
					)}
				</div>
			</div>
				...
				<div className="space-y-1">
					<div className="flex items-center justify-between">
						...
						{skippedGoroutines.size > 0 && (
							<button
								...
								title={t("devProfiler.clearHidden")}
							>
								<RotateCcw className="h-2.5 w-2.5" />
								{t("devProfiler.hidden", { count: skippedGoroutines.size })}
							</button>
						)}
					</div>
					...
					{problemGoroutines.length === 0 && skippedGoroutines.size > 0 && (
						<div className="rounded bg-zinc-800/30 py-2 text-center text-[10px] text-zinc-500">
							{t("devProfiler.allPotentialLeaksHidden")}
						</div>
					)}
					{problemGoroutines.length === 0 &&
						...
						(summary.long_waiting > 0 || summary.potentially_stuck > 0) && (
							<div className="rounded bg-zinc-800/30 px-2 py-2 text-center text-[10px] text-zinc-500">
								{summary.long_waiting > 0 && summary.potentially_stuck > 0
									? t("devProfiler.longWaitingAndStuck", {
											longWaiting: summary.long_waiting,
											stuck: summary.potentially_stuck,
										})
									: summary.long_waiting > 0
										? t("devProfiler.longWaiting", { count: summary.long_waiting })
			...
			{problemGoroutines.length === 0 && summary.long_waiting === 0 && summary.potentially_stuck === 0 && (
				<div className="rounded bg-zinc-800/30 py-2 text-center text-[10px] text-zinc-500">{t("devProfiler.noLeaks")}</div>
			)}
		</div>
	);
}
...
export function DevProfiler(): React.ReactNode {
	...
	return (
		<div className="fixed right-4 bottom-4 z-50 w-[420px] overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900 font-mono text-xs text-zinc-100 shadow-2xl">
			...
			<div className="flex items-center justify-between border-b border-zinc-700 bg-zinc-800 px-3 py-2">
				...
				<div className="flex items-center gap-1">
					<button
						...
						title={isExpanded ? t("devProfiler.collapse") : t("devProfiler.expand")}
					>
			...
			{Boolean(error) && <div className="border-b border-zinc-700 bg-red-900/30 px-3 py-2 text-red-300">{t("devProfiler.loadError")}</div>}
				...
				<div className="custom-scrollbar max-h-[70vh] overflow-x-hidden overflow-y-auto">
					...
					<div className="border-b border-zinc-700 p-3">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-zinc-400">{t("devProfiler.cpuUsageLast5Min")}</span>
						</div>
						<div className="h-24">
					...
					<div className="border-b border-zinc-700 p-3">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-zinc-400">{t("devProfiler.memoryLast5Min")}</span>
						</div>
						<div className="h-24">
							<ResponsiveContainer width="100%" height="100%">
							...
							{(data.top_allocations ?? []).map((alloc, i) => (
								<div key={i} className="flex items-center justify-between rounded bg-zinc-800 px-2 py-1">
									...
									<div className="flex flex-col items-end">
										...
										<span className="text-[10px] text-zinc-500">{t("devProfiler.allocs", { count: alloc.count.toLocaleString() })}</span>
									</div>
								</div>
							))}
						</div>
					...
					<div className="border-t border-zinc-700 bg-zinc-800 px-3 py-2 text-[10px] text-zinc-500">
						{t("devProfiler.cpus")}: {data.runtime.num_cpu} | GOMAXPROCS: {data.runtime.gomaxprocs} | GC: {data.runtime.num_gc} |{" "}
						{t("devProfiler.objects")}: {data.memory.heap_objects.toLocaleString()}
					</div>
				</div>
			)}
			...
			{!isExpanded && data && (
				<div className="flex items-center justify-between bg-zinc-800/50 px-3 py-2">
					...
					<span className="text-zinc-400">
						{t("devProfiler.heap")}: {formatBytes(data.memory.heap_inuse)}
					</span>
					<span className="text-zinc-400">
						{t("devProfiler.goroutines")}: {data.runtime.num_goroutine}
					</span>
				</div>
			)}
		</div>
