	...
	return (
		...
		>
			{supportsVision && isDragging && (
				<div className="bg-background/80 border-primary absolute inset-0 z-50 flex items-center justify-center rounded-sm border-2 border-dashed backdrop-blur-sm">
					<div className="text-primary flex flex-col items-center gap-1">
						...
						<span className="text-xs font-medium">{t("prompts.playground.dropFilesToAttach")}</span>
					</div>
				</div>
			)}
			<div className="mb-1 flex items-center">
				...
				{supportsVision && inputRole === "user" && (
					<div className="ml-auto">
						...
						<button
							type="button"
							aria-label={t("prompts.playground.attachFile")}
			...
			<div className="relative">
				<Textarea
					placeholder={t("prompts.playground.typeMessage")}
					value={userInput}
					ref={userInputRef}
						...
						<Tooltip>
							<TooltipTrigger asChild>
								...
								>
									...
									{t("prompts.playground.run")}
								</Button>
							</TooltipTrigger>
							<TooltipContent side="top">
								{!canRun ? <span>{t("prompts.playground.selectProviderAndModel")}</span> : <span>{t("prompts.playground.runPrompt")}</span>}
