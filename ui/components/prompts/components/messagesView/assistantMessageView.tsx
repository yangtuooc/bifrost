		...
		>
						...
						<Tooltip>
							...
							<TooltipContent side="bottom">
								<div className="flex flex-col gap-0.5 text-xs tabular-nums">
									<span>
										<span className="inline-block w-12">{t("prompts.messages.inputLabel")}</span> {usage.prompt_tokens}{" "}
										{t("prompts.messages.tokens")}
									</span>
									<span>
										<span className="inline-block w-12">{t("prompts.messages.outputLabel")}</span> {usage.completion_tokens}{" "}
										{t("prompts.messages.tokens")}
									</span>
									<span>
										<span className="inline-block w-12">{t("prompts.messages.totalLabel")}</span> {usage.total_tokens}{" "}
										{t("prompts.messages.tokens")}
									</span>
								</div>
							</TooltipContent>
			...
			<div>
				{isStreaming && isEmpty ? (
					...
					<div className="text-muted-foreground min-h-[20px] text-sm italic">{t("prompts.messages.enterAssistantMessage")}</div>
				) : contentIsJson ? (
					<CodeEditor
						wrap
						code={formattedJson}
